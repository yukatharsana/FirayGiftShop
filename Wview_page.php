...
if(!isset($user_id)){
   header('location:login.php');
    ...
    $check_wishlist_numbers = mysqli_query($connect, "SELECT * FROM `wishlist` WHERE Product_Quote = '$product_quote' AND User_ID = '$user_id'") or die('query failed');

    $check_cart_numbers = mysqli_query($connect, "SELECT * FROM `cart` WHERE Product_Quote = '$product_quote' AND User_ID = '$user_id'") or die('query failed');
    ...
    if(mysqli_num_rows($check_wishlist_numbers) > 0){
        ...
        mysqli_query($connect, "INSERT INTO `wishlist`(user_id, Product_ID, Product_Quote, Product_Price, Product_Image) VALUES('$user_id', '$product_id', '$product_quote', '$product_price', '$product_image')") or die('query failed');
...
<input type ="text" placeholder="Optional"  id="color" name="color" value=""> 
<br>
